...
- **Output**: `cross_results$integration` — classified gene/CpG pairs plus summary counts per concordance category (`cx$summary$counts`), consumed by `build_cx_context()`'s bespoke integration formatter in `R/modules_index.R` and by Biomarker Convergence.
