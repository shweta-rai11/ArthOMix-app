...
- **`crossomics_integration_plots.R`** — `cx_empty_state` (shared empty-state UI across all 3 analytical stages) plus integration-specific plots including `cx_gene_cpg_network_plot()` (also reused by `R/multiomics/multiomics_mapping_plots.R`).
