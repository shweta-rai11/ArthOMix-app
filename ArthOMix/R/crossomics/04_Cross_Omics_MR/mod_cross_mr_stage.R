...
## MR Evidence: viewer/classifier for externally-run single-instrument mQTL-MR results; computes no MR itself.
...
mod_cross_mr_stage_server <- function(id, cross_dataset, cross_results = NULL, app_session = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$load_evidence_upload, {
      ...
      ## Log this handler like load_mr/load_mr_upload, so swapping the gene-level evidence table leaves a trace.
      arthomix_provenance_push(arthomix_provenance_record(
        module = "mod_cross_mr_stage",
