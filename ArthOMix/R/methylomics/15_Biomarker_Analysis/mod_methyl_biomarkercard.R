...
bc_single_cpg_roc <- function(beta_row, group_vec, case_label, control_label) {
  ...
  ## max(AUC, 1 - AUC). Never reuse it for held-out data (see bc_single_cpg_cv for the honest version).
...
bc_single_cpg_cv <- function(beta_row, group_vec, case_label, control_label, k = 5, seed = 1234) {
  ...
  for (f in seq_len(k_eff)) {
    ...
    ## re-estimating it on the pooled out-of-fold labels would turn an anti-predictive CpG into an AUC >= 0.5,
  ...
  roc_pooled <- tryCatch(pROC::roc(y_used, score_used, levels = c(control_label, case_label), direction = "<", quiet = TRUE), error = function(e) NULL)
