...
## Methylomics Dataset tab: loads the shared `methyl_dataset` reactiveValues every Methylomics sub-module reads from.
        ...
        box(
          ...
          radioButtons(ns("upload_mode"), NULL, width = "100%",
                       choices = c("Preloaded whole-blood dataset (sex-stratified RA)" = "preloaded",
                                   "Upload: beta or M-value matrix (CSV/TSV)" = "matrix",
                                   "Upload: raw IDAT files (.idat)" = "idat"),
      ...
      column(
        ...
        conditionalPanel(
          ...
          box(
            ...
            fileInput(ns("sheet_file"), "Sample sheet / phenotype metadata (optional, CSV/TSV)", accept = c(".csv", ".tsv", ".txt")),
        ...
        conditionalPanel(
          ...
          box(
            ...
            p(strong("IDAT files"), ": select every ", code("_Grn.idat"), " and ", code("_Red.idat"), " file at once."),
            fileInput(ns("idat_files"), "IDAT files", multiple = TRUE, accept = c(".idat", ".idat.gz")),
            fileInput(ns("idat_sheet_file"), "Sample sheet (optional, CSV/TSV)", accept = c(".csv", ".tsv", ".txt")),
...
mod_methyl_dataset_server <- function(id, methyl_dataset) {
  moduleServer(id, function(input, output, session) {
    ...
    if (isTRUE(ARTHOMIX_ASYNC_AVAILABLE)) {
      ...
      observeEvent(input$load_preloaded_btn, {
        ...
        if (isTRUE(preloaded_matrix_fetched()) || !METH_RAW_DATA_AVAILABLE) {
          ## Already fetched this session, or no big matrix to fetch - skip the background task.
          finish_preloaded_load(preloaded_matrix_cache())
        } else if (!is.null(.arthomix_cache[["meth_default_matrix"]])) {
          ## Another session already cached the matrix at the process level - reuse it instantly.
      ...
      ## Reacts to the background task's status once it resolves (success or error).
      observe({
        ...
        ## Populate the process-level cache from the main process, since the worker's own cache is invisible here.
      ...
      ## No `future`/`promises` installed - loads synchronously instead.
      observeEvent(input$load_preloaded_btn, {
        req(METH_DATA_AVAILABLE)
    ...
    observeEvent(input$load_idat_btn, {
      ...
      ## Prefer the array type recorded on the IDAT files themselves over the dropdown selection.
