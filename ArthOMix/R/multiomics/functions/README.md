...
- **`multiomics_integration_helpers.R`** / **`multiomics_integration_plots.R`** (`mi_*`) — DIABLO/SNF implementation, primarily for `../03_DIABLO_SNF_Integration/` but also the SNF fusion/clustering primitives `../04_SNF_Clustering/`'s own `sfc_snf_run()` delegates to (see `../README.md`).
