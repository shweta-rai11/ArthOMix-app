    ...
    fluidRow(
      column(
        ...
        box(
          ...
          p(class = "submodule-desc", "MOFA2 (Argelaguet et al. 2018/2020): unsupervised latent-factor model, X_m ~ Z W_m^T + E_m. Factors capture variation, not disease status or causality."),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$mofa_availability_note <- renderUI({
      ...
      div(class = "empty-note", style = "border-color: var(--color-warning, #eda100);", icon("triangle-exclamation"),
          " MOFA2 is not installed in this deployment - the Integration step below is unavailable.")
    })
    ...
    if (isTRUE(ARTHOMIX_ASYNC_AVAILABLE)) {
      ...
      observeEvent(input$train_btn, {
        req(MULTI_MOFA_AVAILABLE, live_state$mats)
        model_state$error <- NULL
        showNotification("Training MOFA2 synchronously - the app will be briefly unresponsive.", type = "message", duration = 5)
        res <- run_training()
        if (!isTRUE(res$ok)) { model_state$error <- res$error; return() }
        model_state$model <- res$model
        model_state$variance_df <- multi_live_mofa_variance_df(res$model)
    ...
    output$variance_ui <- renderUI({
      ...
      tagList(
        ...
        p(class = "submodule-desc", tags$em("How to read this: ", "each bar segment is the fraction of one view's variance captured by that factor. High variance does not imply clinical relevance."))
      )
    })
    ...
    output$scores_ui <- renderUI({
      ...
      tagList(
        ...
        p(class = "submodule-desc", tags$em("How to read this: ", "samples close together have similar factor scores. Check grouping against metadata before calling it meaningful."))
      )
    })
    ...
    output$loadings_ui <- renderUI({
      ...
      tagList(
        ...
        p(class = "submodule-desc", tags$em("How to read this: ", "larger absolute loading means the feature contributes more strongly to that factor - not evidence of clinical importance."))
      )
    })
    ...
    output$dl_bundle <- downloadHandler(
      ...
      content = function(file) {
        ...
        writeLines(c(
          "# MOFA2 Integrated Analysis - reproducibility",
