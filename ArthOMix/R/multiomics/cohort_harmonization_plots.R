## R/multiomics/cohort_harmonization_plots.R
...
## Pairwise overlap matrix (ch_pairwise_overlap_matrix()) as a tile heatmap.
...
## Modality x sample presence/absence tile plot (spec section 13's "data
## completeness" figure). Capped to a legible number of samples.
...
## PCA scatter (from the already-computed multi_live_pca() object, reused
## as-is) with one sample highlighted - lets the Sample Explorer tab show
## where a specific sample sits relative to the rest, not just its raw
## metadata row.
...
## Simple count bar charts - only rendered when the column was actually
## detected/selected, never fabricated.
