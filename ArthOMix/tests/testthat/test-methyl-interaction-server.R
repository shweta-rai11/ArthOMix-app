...
## for methylomics' lack of a fixed group/sex column name (a group-column
## picker + mod_methyl_dmp_sex_col() auto-detection instead) and its
## beta -> M-value fit convention (mod_methyl_dmp.R's live engine).
##
## This submodule is inherently a "both sexes present" analysis (Case 1 of
## the sex-stratification test matrix); Cases 2-4 (male-only/female-only/
## no-sex-metadata) are covered here as the "controls UI correctly blocks
## running at all" behavior this module is designed to have in those
## situations.
...
test_that("Case 1 (M+F present): the interaction model fits on M-values and reports a coefficient name, significance counts, and a beta-scale delta", {
  ...
  shiny::testServer(mod_methyl_interaction_server, args = list(id = "int", methyl_dataset = dataset, methyl_results = methyl_results), {
    ...
    ## Extra assertion beyond the transcriptomics template: confirm the fit
    ## ran on M-values (the M-value scale is unbounded, unlike a beta-scale
    ## fit which would be constrained to roughly [-1, 1]) while a companion
    ## beta-scale delta-beta column is also reported for interpretability.
    expect_false(isTRUE(res$is_m_scale))  ## fixture is beta-scale input -> logit-transformed before the fit
    expect_true("dbeta_interaction" %in% colnames(df))
    expect_true(all(abs(df$dbeta_interaction) <= 1, na.rm = TRUE))  ## beta-scale delta must stay within [-1, 1]
...
test_that("fewer than 12 total matching samples across both sexes/groups is rejected", {
  dataset <- methyl_interaction_fixture(n_per_cell = 2)  ## 2*2*2 = 8 total, below 12
  shiny::testServer(mod_methyl_interaction_server, args = list(id = "int", methyl_dataset = dataset, methyl_results = shiny::reactiveValues()), {
    session$setInputs(group_col = "group", ref_group = "HC", comp_group = "RA", ref_sex = "F", comp_sex = "M", padj_cut = 0.05)
    run_click(session)
    err <- tryCatch(fit_result(), error = function(e) e)
