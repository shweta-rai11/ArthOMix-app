...
## (real Stouffer's-Z combined p-value across CpGs), Hyper/Hypo x Up/Down
## classification and Evidence-Level tiering, every advanced filter,
## sample-pairing/correlation (real cor.test), real gene-ID harmonization
## (org.Hs.eg.db), the preloaded DEG/DMP/DMR loaders (real pipeline data),
## sex comparison, and dataset validation. This file is the shared engine
## Multiomics' Gene-CpG Concordance already treats as an already-verified
## building block - these tests give it its own dedicated, thorough
## coverage as this module's home file.
...
test_that("cx_detect_columns() greedily/order-independently assigns each canonical field to a distinct column", {
  ...
  expect_equal(unname(out["pvalue"]), "P.Value")  ## not double-claimed by fdr's own match
})
...
test_that("cx_detect_sample_columns() finds numeric non-metadata columns, excluding common summary-statistic names", {
  ...
  expect_setequal(out, c("S1", "S2"))  ## AveExpr excluded (CX_NON_SAMPLE_NUMERIC_NAMES), gene/logFC claimed
})

## ---- cx_standardize_expression() / cx_dedup_by_gene() -----------------------
...
test_that("cx_dedup_by_gene() keeps the duplicate row with the smallest FDR, falling back to smallest p-value", {
  ...
  expect_equal(out$fdr[out$gene == "A"], 0.01)  ## smallest FDR wins
})

## ---- cx_region_bucket() / cx_region_fine() -----------------------------------
...
## ---- cx_cpg_level_table() / cx_cpg_counts_per_gene() -------------------------
...
test_that("cx_cpg_level_table() flags sig_cpg correctly and labels methylation_direction from dbeta's sign", {
  ...
  expect_equal(out$sig_cpg, c(TRUE, FALSE))  ## cg2 below the |dbeta| threshold
})
...
test_that("cx_cpg_counts_per_gene() correctly tallies per-gene CpG counts by significance/direction/region/island", {
  ...
  expect_equal(a_row$n_cpg_hyper_sig, 1L)  ## cg1 only (cg2 is sig but hypo)
  expect_equal(a_row$n_body, 2L)
  expect_equal(a_row$primary_region, "Body")  ## 2 of 3 CpGs are Body
})

## ---- cx_aggregate_methylation() (real Stouffer's-Z gene-level combination) ----
...
test_that("cx_aggregate_methylation() 'mean' aggregation averages dbeta per gene and combines p-values via real Stouffer's Z (reduces to the single CpG's own p-value for a 1-CpG gene)", {
  ...
  expect_equal(b_row$pvalue, 0.02, tolerance = 1e-8)  ## single-CpG gene -> Stouffer reduces to its own p-value
...
test_that("cx_aggregate_methylation() 'min_fdr'/'max_abs_dbeta' pick the correct representative CpG per gene", {
  ...
  expect_equal(out_dbeta$df$cpg, "cg2")  ## |-0.5| > |0.1|
})
...
test_that("cx_aggregate_methylation() region-restricted methods ('promoter_only' etc.) filter CpGs before aggregating", {
  ...
  expect_true(out$ok)
  expect_equal(out$df$n_probes, 1L)  ## only cg1 (Promoter) retained
...
## ---- cx_classify() / cx_classify_evidence() ----------------------------------
...
test_that("cx_classify() assigns the correct 4-way Hyper/Hypo x Up/Down category only when BOTH layers clear significance", {
  ...
  ## Row 1: log2fc=+2 (Up), dbeta=-0.3 (Hypo) -> "Hypo + Up". Row 2: log2fc=-2
  ## (Down), dbeta=+0.3 (Hyper) -> "Hyper + Down". Rows 3-4 are same-direction
  ## (Hyper+Up / Hypo+Down); row 5 doesn't clear the expression threshold.
...
test_that("cx_classify_evidence() requires has_correlation=TRUE AND a significant negative correlation for 'Strong candidate'", {
  ...
  expect_equal(as.character(cx_classify_evidence(df, has_correlation = FALSE)), "Moderate candidate")  ## correlation ignored when not computed
})
...
test_that("cx_filter_by_category()/cx_filter_by_region()/cx_filter_by_island()/cx_filter_by_evidence()/cx_filter_by_min_cpg()/cx_filter_by_correlation_direction() each apply correctly and no-op safely when their column is absent", {
  ...
  expect_equal(cx_filter_by_region(df, NULL), df)  ## no-op
...
test_that("cx_build_gene_sample_matrix() averages duplicate-id rows into one, preserves unique-id rows unchanged", {
  ...
  expect_equal(unname(out["A", "S1"]), 2)  ## mean(1,3)
  expect_equal(unname(out["B", "S2"]), 6)
})

## ---- cx_detect_sample_pairing() / cx_gene_correlation() (real cor.test) ------
...
test_that("cx_detect_sample_pairing() is conservative: paired=FALSE without both sides having per-sample columns or enough overlap", {
  ...
  expect_false(out$paired)  ## only 2 shared, need >=3
...
test_that("cx_gene_correlation() computes a real per-gene correlation matching stats::cor.test() directly", {
  ...
  meth["G1", ] <- -expr["G1", ] + rnorm(10, sd = 0.05)  ## strong real negative correlation
...
## ---- cx_get_region_annotation() (real 450K/EPIC manifest) --------------------
...
test_that("cx_get_region_annotation() loads real chr/pos/gene/region/island columns from the real 450K manifest and caches on repeat calls", {
  ...
  out2 <- cx_get_region_annotation("450K")  ## cached path
...
## ---- Preloaded DEG/DMP/DMR loaders (real pipeline data) -----------------------
...
## ---- cx_harmonize_gene_ids() (real org.Hs.eg.db) - ambiguous-alias case -------
...
test_that("cx_validate_dataset() reports real per-check pass/fail and readiness from whatever's actually loaded", {
  ...
  expect_true(out$compatibility[[2]]$ok)  ## overlap: "A" is in both
