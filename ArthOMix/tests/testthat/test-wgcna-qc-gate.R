...
## "all genes" filter method - the default for this project's own dataset -
## has no incidental variance floor of its own, so a constant-expression gene
## could reach pickSoftThreshold()/blockwiseModules() undetected.
...
test_that("a zero-variance gene is flagged and dropped before network construction", {
  ...
  expr["gene1", ] <- 5  ## constant across every sample
