...
## ---- multi_sample_harmonization() ----------------------------------------------

test_that("multi_sample_harmonization() correctly tallies matched/RNA-only/meth-only/neither across a matching table", {
  matching_df <- data.frame(
    sample_id = paste0("S", 1:10),
    RNA_available_PBMC = c(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE),
    methylation_available = c(TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE),
    sex = c(rep("female", 5), rep("male", 5))
  )
  out <- multi_sample_harmonization(matching_df)
  expect_true(isTRUE(out$ok))
  expect_equal(out$n_total, 10L)
  expect_equal(out$n_rna, 7L)
  expect_equal(out$n_meth, 7L)
  expect_equal(out$n_matched, 5L)   ## S1-S5: both TRUE
  expect_equal(out$n_rna_only, 2L)  ## S6, S7: rna only
  expect_equal(out$n_meth_only, 2L) ## S8, S9: meth only
  expect_equal(out$n_neither, 1L)   ## S10
})

test_that("multi_sample_harmonization() fails soft with a clear error when the expected availability columns are absent", {
  out <- multi_sample_harmonization(data.frame(sample_id = paste0("S", 1:3), has_rna = c(TRUE, TRUE, FALSE)))
  expect_false(out$ok)
  expect_true(nzchar(out$error))
})

test_that("multi_sample_harmonization() reports by_sex as the sex breakdown of MATCHED (both-available) samples only", {
  matching_df <- data.frame(
    sample_id = paste0("S", 1:6),
    RNA_available_PBMC = c(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE),
    methylation_available = c(TRUE, TRUE, FALSE, TRUE, TRUE, FALSE),
    sex = c("female", "female", "female", "male", "male", "male")
  )
  out <- multi_sample_harmonization(matching_df)
  expect_equal(out$n_matched, 4L)  ## S1,S2,S4,S5
  expect_true(!is.null(out$by_sex))
  by_sex <- setNames(out$by_sex$Freq, out$by_sex$sex)
  expect_equal(unname(by_sex[["female"]]), 2L)  ## S1,S2 matched
  expect_equal(unname(by_sex[["male"]]), 2L)    ## S4,S5 matched
  expect_equal(sum(out$by_sex$Freq), out$n_matched)
})
...
## ---- multi_concordance_pattern_tally() -----------------------------------------

test_that("multi_concordance_pattern_tally() tallies the biological_pattern column correctly", {
  df <- data.frame(biological_pattern = c("Concordant", "Concordant", "Discordant", "Concordant", "Ambiguous"))
  out <- multi_concordance_pattern_tally(df)
  expect_equal(as.integer(out[out$pattern == "Concordant", "n"]), 3L)
  expect_equal(as.integer(out[out$pattern == "Discordant", "n"]), 1L)
  expect_equal(sum(out$n), nrow(df))
})

test_that("multi_concordance_pattern_tally() returns NULL when biological_pattern column is absent or df is NULL", {
  expect_null(multi_concordance_pattern_tally(data.frame(other_col = 1:3)))
  expect_null(multi_concordance_pattern_tally(NULL))
})
